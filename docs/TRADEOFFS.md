...
| **DD-11** Sync SQLAlchemy, run inside the request | No async colouring, no broker, no worker | Measured: ten thousand records a side takes **3 seconds against SQLite and 18 against Postgres**, because every row now crosses a connection rather than a file handle. Eighteen seconds inside a web request is already poor, with no progress indication, and a timeout mid-run leaves the analyst guessing | **Moderate** |
